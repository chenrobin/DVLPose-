...
def reset_config(config,args):
    ...
    if args.shift_heatmap:
        config.TEST.SHIFT_FILE = args.shift_heatmap
...
def main():
    ...
    if config.TEST.MODEL_FILE:
        ...
        model.load_state_dict(torch.load(config.TEST_MODEL_FILE))
    else:
    ...
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.460],
                                     std=[0.229, 0.224, 0.225])
    valid_dataset = eval('dataset.'+config.DATASET.DATASET+'.COCODataset')(
        config,
        config.DATASET.ROOT,
